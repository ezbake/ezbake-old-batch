...
import ezbake.services.amino.thrift.TQueryEntry;
...
public class RunQuery extends Command {
    ...
    public Integer execute() throws Exception {
        ...
        THypothesis createResult = aminoClient.createHypothesis(createHypothesis(), securityToken);

        TQueryResult queryResult = aminoClient.createResult(securityToken.getValidity().getIssuedTo(),
                createResult.getId(), 200, loadedProps.getProperty(Props.Justification.pname()), securityToken);

        List<TQueryEntry> results = queryResult.getResult_set();
        output.println("Got back " + results.size() + " results:");

        // Sort the results to make it easier to see what you got back
        Collections.sort(results, new Comparator<TQueryEntry>() {
            @Override
            public int compare(TQueryEntry o1, TQueryEntry o2) {
                return o1.getBucketName().compareToIgnoreCase(o2.getBucketName());
            }
        });

        // Dump the sorted results
        for(TQueryEntry result : results){
            output.println(result.getBucketName());
        }
